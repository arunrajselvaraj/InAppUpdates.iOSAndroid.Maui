...
public static class AndroidUserInterface
{
    /// <summary>
    ...
    public static void ShowShortToast(string text)
    {
        try
        {
            ...
            {
                AndroidHandler.Options.DebugAction($"Cannot show toast - Platform.AppContext is null: {text}");
                return;
            }
        ...
        {
            AndroidHandler.Options.DebugAction($"Failed to show toast '{text}': {ex}");
        }
    }
    ...
    {
        AndroidHandler.Options.DebugAction($"{debugMessage}, falling back to toast and auto-triggering action");
        ...
        if (clickHandler is not null && fallbackView is not null)
        {
            ...
            {
                AndroidHandler.Options.DebugAction($"Error auto-executing snackbar action: {actionEx}");
            }
        }
    }
    ...
    {
        AndroidHandler.Options.DebugAction($"Snackbar {errorType}: {ex}");
        ShowShortToast(fallbackMessage);
        ...
        if (clickHandler is not null && view is not null)
        {
            ...
            {
                AndroidHandler.Options.DebugAction($"Error executing snackbar action: {actionEx}");
            }
        }
    }
